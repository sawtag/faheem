...
from app.prompts.comparables_precedents_prompts import TRADING_COMPS_PROMPT


class TradingCompsAgent(BaseAnalysisAgent):
    name = "trading_comps"

    async def analyze(self, state: FinancialAnalysisState) -> AgentOutput:
        llm = get_llm()
        response = await llm.ainvoke([
            SystemMessage(content=TRADING_COMPS_PROMPT),
