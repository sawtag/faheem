...
from app.prompts.comparables_precedents_prompts import PRECEDENT_TRANSACTIONS_PROMPT


class PrecedentTransactionsAgent(BaseAnalysisAgent):
    name = "precedent_transactions"

    async def analyze(self, state: FinancialAnalysisState) -> AgentOutput:
        llm = get_llm()
        response = await llm.ainvoke([
            SystemMessage(content=PRECEDENT_TRANSACTIONS_PROMPT),
