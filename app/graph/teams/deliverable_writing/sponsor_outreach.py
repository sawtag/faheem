...
from app.prompts.deliverable_writing_prompts import SPONSOR_OUTREACH_PROMPT


class SponsorOutreachAgent(BaseAnalysisAgent):
    name = "sponsor_outreach"

    async def analyze(self, state: FinancialAnalysisState) -> AgentOutput:
        llm = get_llm()
        response = await llm.ainvoke([
            SystemMessage(content=SPONSOR_OUTREACH_PROMPT),
        ...
        return AgentOutput(
            ...
            confidence=0.83,
            metadata={"team": "deliverable_writing", "document_type": "teaser"},
        )
