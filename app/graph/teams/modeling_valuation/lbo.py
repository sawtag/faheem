...
from app.prompts.modeling_valuation_prompts import LBO_PROMPT


class LBOAgent(BaseAnalysisAgent):
    name = "lbo"

    async def analyze(self, state: FinancialAnalysisState) -> AgentOutput:
        llm = get_llm()
        response = await llm.ainvoke([
            SystemMessage(content=LBO_PROMPT),
        ...
        return AgentOutput(
            ...
            confidence=0.80,
            metadata={"team": "modeling_valuation", "model_type": "lbo"},
        )
